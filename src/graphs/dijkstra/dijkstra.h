  ...
  while(predecessor != start_vertex)
  {
    ...
    if (iter == map_path_predecessor.end())
    {
      ...
      std::cerr << "get_path_from_predecessors(): predecessor not found for vertex:"
        << predecessor
        << ", start_vertex=" << start_vertex << ", end_vertex=" << end_vertex << std::endl;
      */
      break;
    }
...
dijkstra_compute_shortest_paths(const type_vec_nodes& vertices,
  type_num start_vertex, type_num end_vertex,
  bool stop_at_end = false)
{
  ...
  while(!heap.empty())
  {
    ...
    {
      const auto path = get_path_from_predecessors(start_vertex, end_vertex,
  ...
  for (type_num i = 0; i < vertices_size; ++i)
  {
    ...
    const auto path = get_path_from_predecessors(start_vertex, i, map_path_predecessor);
...
std::vector<ShortestPath>
dijkstra_compute_shortest_paths(const type_vec_nodes& vertices, type_num start_vertex)
{
...
{
  const auto shortest_paths = dijkstra_compute_shortest_paths(vertices, start_vertex, end_vertex, true /* stop at end_vertex */);
  if (shortest_paths.empty()) {
    std::cerr << "dijkstra_compute_shortest_path(): dijkstra_compute_shortest_path() failed." << std::endl;
