...
static void
test_small_dest(
  type_num dest_vertex_num, Edge::type_length expected_shortest_path) {
  const auto shortest_path =
    dijkstra_compute_shortest_path(EXAMPLE_GRAPH_SMALL, 0, dest_vertex_num);
...
main() {
  ...
  for (auto dest_vertex_num : dest_vertices) {
    const auto shortest_path = dijkstra_compute_shortest_path(
      EXAMPLE_GRAPH_SMALL, start_vertex, dest_vertex_num);
