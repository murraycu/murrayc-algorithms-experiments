...
public:
   ...
   */
  static SubsetIterator
  create(type_size n, type_size k) {
    SubsetIterator iter;
  ...
  next() {
    ...
    const Byte bits_past_max =
      ~((0x01 << n_) - 1); // For instance, ...111111100 for 2.

    // Gosper's Hack:
    // See https://en.wikipedia.org/wiki/Combinatorial_number_system
    const Byte u = bits_ & -bits_; // extract rightmost bit 1.
    const Byte v =
      u + bits_; // set last non-trailing bit 0, and clear to the right.

    if (v == 0)
      return bits_; // Overflow. Stop because we cannot represent the next
                    // combination in Byte.
   ...
   */
  type_container
  get_subset(const type_container& set) const {
