...
public:
  ...
  edges_type get_edges(unsigned int rotation) const {
    auto result = edges_;
    // Rotate to the right:
    std::rotate(std::rbegin(result), std::rbegin(result) + rotation, std::rend(result));
    return result;
  }
...
static
std::string print_color(Card::Color color) {
  switch (color) {
    case Card::RED:
      return "R";
    case Card::GREEN:
...
public:
  ...
  bool next_card_satisfies_constraints(const cards_type& cards, const solution_type& prefix,
      const Card& card, unsigned int rotation) const {
    const auto prefix_size = prefix.size();
    const auto card_pos = prefix_size; //1 later.
    //std::cout << "  testing: card_pos=" << card_pos << ", rotation=" << rotation << '\n';
    for (unsigned int edge = 0; edge < Card::EDGES_COUNT; ++edge) {
      ...
      //std::cout << "        card edge=" << edge << ", with rotation=" << rotation << ": " << card_edge << "\n";
      if (!edges_match(neighbour_card, neighbour_edge, card, card_edge)) {
        //std::cout << "        conflict.\n";
        return false;
      }

      //std::cout << "        edges match: current: " << print_color(card.edges_[card_edge])
      //  << ", neighbour_edge: " << print_color(neighbour_card.edges_[neighbour_edge]) << '\n';
    }
...
private:
   ...
   */
  static edge_type get_neighbour_edge(edge_type edge) {
    switch (edge) {
      case Card::EDGE_TOP:
   ...
   */
  static edge_type rotated_edge(edge_type edge, unsigned int rotation) {
  ...
  bool has_edge(unsigned int pos, edge_type edge) const {
   const auto x = pos % width_;
   const auto y = pos / height_; //Most compilers will combine the % and / into 1 instruction.
   return has_edge(x, y, edge);
  }

  /** @param edge From 0 to 3, meaning top, right, bottom, and left.
   */
  bool has_edge(unsigned int x, unsigned int y, edge_type edge) const {
  ...
  bool get_neighbour_pos(unsigned int pos, edge_type edge, unsigned int& neighbour_pos) const {
    assert(edge <= Card::EDGES_COUNT);
    neighbour_pos = 0;
  ...
  static bool edges_match(
      const Card& card_a, unsigned int card_a_edge,
      const Card& card_b, unsigned int card_b_edge) {
    return card_a.edges_[card_a_edge] ==
      card_b.edges_[card_b_edge];
  }
...
static
void print_solution(const solution_type& solution,
    ...
    for (unsigned int part = 0; part < 3; ++part) {
      for (unsigned int x = 0; x < width; ++x) {
        ...
        if (part == 0) {
          ...
          std::cout << print_color(edges[Card::EDGE_LEFT]) << " " << print_color(edges[Card::EDGE_RIGHT]);
        } else if (part == 2) {
 ...
 */
static
std::vector<solution_type> get_solutions(const Grid& grid, const std::vector<Card>& cards, const solution_type& prefix) {
  ...
  for(unsigned int i = 0; i < cards_count; ++i) {
    ...
    if (std::find_if(std::begin(prefix), std::end(prefix), 
          [i] (const card_and_rotation& item) {
            return item.first == i;
          }) != std::end(prefix)) {
      continue;
    }
      ...
      if (grid.next_card_satisfies_constraints(cards, prefix, card, rotation)) {
        ...
        */
        ...
        if (solution_size == solution_size_needed) {
          ...
          result.insert(std::end(result), std::begin(solutions), std::end(solutions));
        }
      }
    }
  }
...
static
std::vector<solution_type> get_solutions(const Grid& grid, const std::vector<Card>& cards) {
