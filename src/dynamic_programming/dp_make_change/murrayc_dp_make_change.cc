...
calc_optimal_sub_problem(const type_vec_coins& items, type_size item_number,
  type_value needed_value, type_sub_problems& sub_problems, int level) {
  ++level;
  ...
  //  ", item value=" << item_value << ", needed_value=" << needed_value <<
  //  std::endl;
  ...
  if (iter != map.end()) {
    ...
    // indent(level);
    // std::cout << "(cached) result.coin_count_used=" << result.coin_count_used
    // << std::endl;
  ...
  auto case_dont_use_this_item =
    (item_number == 1 ? SubSolution(COIN_COUNT_INFINITY)
                      : calc_optimal_sub_problem(items, item_number - 1,
                          needed_value, sub_problems, level));
  ...
  if (item_value == needed_value) {
    ...
    // indent(level);
    // std::cout << "Taking case_dont_use_this_item because item_value too big."
    // << std::endl;
    ...
    if (item_number != 1) {
      case_use_this_item = calc_optimal_sub_problem(
        items, item_number - 1, needed_value - item_value, sub_problems, level);
    ...
    // <<
    //    ", case_dont_use_this_item=" <<
    //    case_dont_use_this_item.coin_count_used << std::endl;
    ...
    // to maximize value.
    if (case_use_this_item.coin_count_used <
        case_dont_use_this_item.coin_count_used) {
      // indent(level);
      // std::cout << "Taking case_use_this_item because it is smaller: " <<
      // case_use_this_item.coin_count_used << std::endl;

      result = case_use_this_item;
      result.solution.emplace_back(item_value);
    } else {
      // indent(level);
      // std::cout << "Taking case_dont_use_this_item because it is smaller:" <<
      // case_dont_use_this_item.coin_count_used << std::endl;
  ...
  // indent(level);
  // std::cout << "result.coin_count_used=" << result.coin_count_used <<
  // std::endl;
...
main() {
  ...
  */
  ...
  //
  // The main way to identify the analogue seems to be to identify what is being
  // maximised or minimised.
  //
  // Unlike with the knapsack problem, we want the minimum coin count for the
  // needed value,
  // instead of the maximum value for the available (or less) weigtht.
  //
  // Use a recursive memoization technique, instead of filling a
  // count*needed_value
  ...
  const auto solution =
    calc_optimal_sub_problem(coins, items_count, needed_value, sub_problems, 0);
  if (solution.coin_count_used != COIN_COUNT_INFINITY) {
    std::cout << "solution: coins count: " << solution.coin_count_used
              << std::endl
              << "with solution: ";
  ...
  for (type_value lesser_value = needed_value - 1; lesser_value > 0;
       --lesser_value) {
    std::cout << "Trying: " << lesser_value << std::endl;
    const auto solution_lesser = calc_optimal_sub_problem(
      coins, items_count, lesser_value, sub_problems, 0);
