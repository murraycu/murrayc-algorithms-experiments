...
static
matrix_type create_matrix() {
  return { {0, 0}, {0, 0} };
}

static
const auto& matrix_xy(const matrix_type& matrix, unsigned int x, unsigned int y) {
  return matrix[y][x];
}
static
auto& matrix_xy(matrix_type& matrix, unsigned int x, unsigned int y) {
  return matrix[y][x];
}
  ...
  for (unsigned int result_x = 0; result_x < result_width; ++result_x) {
    for (unsigned int result_y = 0; result_y < result_height; ++result_y) {
      ...
      for (unsigned int a_x = 0; a_x < a_width; ++ a_x) {
        //std::cout << result_x << ", " << result_y << " += " << matrix_xy(a, a_x, result_y) << " * " << matrix_xy(b, result_x, a_x) << '\n';
...
fibonacci(unsigned int n) {
  ...
  // See https://www.reddit.com/r/cpp/comments/4lpdwz/c_weekly_ep13_fibonacci_youre_doing_it_wrong/
...
int main() {
  ...
  //std::cout << "row0: " << matrix_xy(matrix_mult, 0, 0) << ", " << matrix_xy(matrix_mult, 1, 0) << '\n';
  //std::cout << "row1: " << matrix_xy(matrix_mult, 0, 1) << ", " << matrix_xy(matrix_mult, 1, 1) << '\n';
